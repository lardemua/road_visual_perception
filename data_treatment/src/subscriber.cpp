...
private:
  ...
  inline void drawSpline(cv::Mat inImage,
                         const std::vector<cv::Point> &splinePoints,
                         const uint32_t &thickness, const cv::Scalar &color);
  inline std::vector<cv::Point2f>
  ROS2CvPoint2f(std::vector<geometry_msgs::Point32> &input);
  inline std::vector<cv::Point>
  cvtCvPoint2f2CvPoint(const std::vector<cv::Point2f> &input);
};
...
get_lines::get_lines() : it(n) {
  ...
  sub = it.subscribe("camera/image_raw", 10, &get_lines::imagereceiveCallback,
                     this);
  sub_lanes = n.subscribe("lane_detector/lane", 10,
                          &get_lines::lanereceiveCallback, this);
}
...
void get_lines::Looping() {
  ...
  if (current_image && (left_spline.size() > 0) &&
      (right_spline.size() > 0)) //&& (guide_spline.size() > 0))
  {
...
void get_lines::Publishers() { finalimage.publish(processed_img); }
...
inline std::vector<cv::Point2f>
get_lines::ROS2CvPoint2f(std::vector<geometry_msgs::Point32> &input) {
...
inline std::vector<cv::Point>
get_lines::cvtCvPoint2f2CvPoint(const std::vector<cv::Point2f> &input) {
...
inline void get_lines::drawSpline(cv::Mat inImage,
                                  const std::vector<cv::Point> &splinePoints,
...
// cvtCvPoint2f2CvPoint(guide_spline_pix_float);
/**
...
void get_lines::drawingPolygn() {
  ...
  // drawSpline(processedImage, left_spline, 1, Scalar(B_channel, G_channel,
  // R_channel)); drawSpline(processedImage, right_spline, 1, Scalar(B_channel,
  // G_channel, R_channel));
  ...
  // Debugger
  // cout<< "Minimos:"<< endl;
  // cout << "Left lane: (row_min,col_min)= " << row_min_left_lane << "," <<
  // col_min_left_lane << endl; cout << "Right lane: (row_min,col_min)=
  // "<<row_min_right_lane<<","<<col_min_right_lane<<endl;

  // cout << "Maximos:"<<endl,
  // cout << "Left lane: (row_max,col_max)=
  // "<<row_max_left_lane<<","<<col_max_left_lane<<endl; cout <<"Right lane:
  // (row_max_col_max)= "<<row_max_right_lane<<","<<col_max_right_lane<<endl;
  ...
  // line(processedImage, min_ll, min_rl, Scalar(B_channel, G_channel,
  // R_channel), 1); line(processedImage, max_ll, max_rl, Scalar(B_channel,
  // G_channel, R_channel), 1); floodFill(processedImage, init_fill,
  // Scalar(B_channel, G_channel, R_channel));
  line(processedImage, min_ll, min_rl, 255, 1);
  line(processedImage, max_ll, max_rl, 255, 1);
  floodFill(processedImage, init_fill, 255);

  processed_img =
      cv_bridge::CvImage{current_image->header, "mono8", processedImage}
          .toImageMsg();
}

/**
 * @brief This callback is the one that receives the lanes and show images with
 * the filled area
 *
 */
