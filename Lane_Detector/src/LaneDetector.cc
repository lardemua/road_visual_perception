...
void mcvGetGaussianKernel(CvMat *kernel, unsigned char w, FLOAT sigma)
{
  ...
  {
    CV_MAT_ELEM(*kernel, FLOAT_MAT_ELEM_TYPE, int(i + w), 0) = (FLOAT_MAT_ELEM_TYPE)exp(-(.5 / sigma) * (i * i));
  }
}
 ...
 */
void getIpmMap(CvMat *inImage, CameraInfo *cameraInfo, IPMInfo *ipmInfo, FLOAT_POINT2D vp, LaneDetectorConf *lanesConf,
               list<CvPoint> *outPixels, list<CvPoint> *inPixels, list<CvPoint> *ipm_out_of_area, CvMat *uvGrid)
{
  // input size
 ...
 *
 */
void getLines(const CvMat *image, LineType lineType, vector<Line> &lines, vector<float> &lineScores,
              LaneDetectorConf *lineConf)
{
  mcvGetHVLines(image, &lines, &lineScores, lineType,
                6,  // stopLinePixelHeight,
                lineConf->binarize, lineConf->localMaxima, lineConf->detectionThreshold, lineConf->smoothScores);
 ...
 */
void mcvGetHVLines(const CvMat *inImage, vector<Line> *lines, vector<FLOAT> *lineScores, LineType lineType,
                   FLOAT linePixelWidth, bool binarize, bool localMaxima, FLOAT detectionThreshold, bool smoothScores)
{
  ...
  float smoothp[] = { 0.000003726653172, 0.000040065297393, 0.000335462627903, 0.002187491118183, 0.011108996538242,
                      0.043936933623407, 0.135335283236613, 0.324652467358350, 0.606530659712633, 0.882496902584595,
                      1.000000000000000, 0.882496902584595, 0.606530659712633, 0.324652467358350, 0.135335283236613,
                      0.043936933623407, 0.011108996538242, 0.002187491118183, 0.000335462627903, 0.000040065297393,
                      0.000003726653172 };
  int smoothWidth = 21;
...
#define LOCAL_MAX_IGNORE (int(MAX_IGNORE / 4))
  ...
  for (int i = 0; i < (int)sumLinesMax.size(); i++)
  {
    ...
    double maxLocAcc =
        mcvGetLocalMaxSubPixel(CV_MAT_ELEM(sumLines, FLOAT_MAT_ELEM_TYPE, MAX(sumLinesMaxLoc[i] - 1, 0), 0),
...
void mcvIntersectLineWithBB(const Line *inLine, const CvSize bbox, Line *outLine)
{
  ...
  if (!(startInside && endInside))
  {
    ...
    if (!startInside)
    {
      ...
      for (i = 0; i < 4 && cont; i++)
      {
        if (t[i] >= 0 && t[i] <= 1 && mcvIsPointInside(pts[i], bbox) &&
            !(pts[i].x == outLine->endPoint.x && pts[i].y == outLine->endPoint.y))
        {
    ...
    if (!endInside)
    {
      ...
      {
        if (t[i] >= 0 && t[i] <= 1 && mcvIsPointInside(pts[i], bbox) &&
            !(pts[i].x == outLine->startPoint.x && pts[i].y == outLine->startPoint.y))
        {
 ...
 */
void mcvGetRansacLines(const CvMat *im, vector<Line> &lines, vector<float> &lineScores, LaneDetectorConf *lineConf,
                       LineType lineType)
{
  ...
  vector<CvRect> boxes;
  mcvGetLinesBoundingBoxes(lines, lineType, cvSize(width, height), boxes);
  ...
  for (int i = 0; i < (int)boxes.size(); i++)  // lines
  {
    ...
    float lineScore;
    Line line;
    mcvFitRansacLine(subimage, lineConf->ransacLineNumSamples, lineConf->ransacLineNumIterations,
                     lineConf->ransacLineThreshold, lineConf->ransacLineScoreThreshold, lineConf->ransacLineNumGoodFit,
                     lineConf->getEndPoints, lineType, &line, lineRTheta, &lineScore);
...
void mcvSetMat(CvMat *inMat, CvRect mask, double val)
{
  ...
  if (rect.x < inMat->width && rect.y < inMat->height && rect.x >= 0 && rect.y >= 0 && rect.width > 0 &&
      rect.height > 0)
  {
...
void mcvGroupLines(vector<Line> &lines, vector<float> &lineScores, float groupThreshold, CvSize bbox)
{
  ...
  while (!stop)
  {
    ...
    vector<float>::iterator ir, jr, itheta, jtheta, minIr, minJr, minItheta, minJtheta, iscore, jscore, minIscore,
        minJscore;
    // compute pairwise distance between detected maxima
    for (ir = rs.begin(), itheta = thetas.begin(), iscore = lineScores.begin(); ir != rs.end();
         ir++, itheta++, iscore++)
      for (jr = ir + 1, jtheta = itheta + 1, jscore = iscore + 1; jr != rs.end(); jr++, jtheta++, jscore++)
      {
        // add pi if neg
  ...
  for (int i = 0; i < (int)rs.size(); i++)
  {
    ...
    for (iscore = lineScores.begin(), iline = lines.begin(); iscore != lineScores.end() && newScores[i] <= *iscore;
         iscore++, iline++)
      ;
...
void mcvLineXY2RTheta(const Line &line, float &r, float &theta)
{
  ...
  else
  {
    ...
    theta = atan2(line.endPoint.x - line.startPoint.x, line.startPoint.y - line.endPoint.y);
...
void mcvGroupBoundingBoxes(vector<CvRect> &boxes, LineType type, float groupThreshold)
{
  ...
  while (cont)
  {
      ...
      for (j = i + 1; j != boxes.end(); j++)
      {
        switch (type)
        {
          case LINE_VERTICAL:
            ...
            // i.e. (x12 - x21) / (x22 - x21)
            overlap =
                i->x < j->x ? (i->x + i->width - j->x) / (float)j->width : (j->x + j->width - i->x) / (float)i->width;
          ...
          case LINE_HORIZONTAL:
            ...
            // i.e. (y12 - y21) / (y22 - y21)
            overlap = i->y < j->y ? (i->y + i->height - j->y) / (float)j->height :
    ...
    if (maxOverlap >= groupThreshold)
    {
      ...
      *maxI = cvRect(min((*maxI).x, (*maxJ).x), min((*maxI).y, (*maxJ).y), max((*maxI).width, (*maxJ).width),
...
void mcvGroupBoundingBoxesVec(vector<Box> &boxes, LineType type, float groupThreshold)
{
  ...
  while (cont)
  {
      ...
      for (j = i + 1; j != boxes.end(); j++)
      {
        switch (type)
        {
          case LINE_VERTICAL:
            // get one with smallest x, and compute the x2 - x1 / width of smallest
            // i.e. (x12 - x21) / (x22 - x21)
            overlap = i->box.x < j->box.x ? (i->box.x + i->box.width - j->box.x) / (float)j->box.width :
          ...
          case LINE_HORIZONTAL:
            // get one with smallest y, and compute the y2 - y1 / height of smallest
            // i.e. (y12 - y21) / (y22 - y21)
            overlap = i->box.y < j->box.y ? (i->box.y + i->box.height - j->box.y) / (float)j->box.height :
    ...
    if (maxOverlap >= groupThreshold)
    {
      ...
      // combine the two boxes
      *maxI = Box{ cvRect(min((*maxI).box.x, (*maxJ).box.x), min((*maxI).box.y, (*maxJ).box.y),
                          max((*maxI).box.width, (*maxJ).box.width), max((*maxI).box.height, (*maxJ).box.height)),
                   lin };
      // delete the second one
      boxes.erase(maxJ);
    }
 ...
 *
 */
void mcvFitRansacLine(const CvMat *image, int numSamples, int numIterations, float threshold, float scoreThreshold,
                      int numGoodFit, bool getEndPoints, LineType lineType, Line *lineXY, float *lineRTheta,
                      float *lineScore)
{
  ...
  float dist, score;
  Line curEndPointLine = { { -1., -1. }, { -1., -1. } }, bestEndPointLine = { { -1., -1. }, { -1., -1. } };
  ...
  for (int i = 0; i < numIterations; i++)
  {
    ...
    for (int j = 0; j < numSamples; j++)
    {
      ...
      // put point
      CV_MAT_ELEM(*samplePoints, float, 0, j) = CV_MAT_ELEM(*points, float, 0, CV_MAT_ELEM(*randInd, int, j, 0));
      CV_MAT_ELEM(*samplePoints, float, 1, j) = CV_MAT_ELEM(*points, float, 1, CV_MAT_ELEM(*randInd, int, j, 0));
    }
    ...
    if (numClose >= numGoodFit)
    {
      ...
      for (int j = 0; j < points->cols; j++)
        if (CV_MAT_ELEM(*pointIn, char, 0, j))
        {
          CV_MAT_ELEM(*fitPoints, float, 0, k) = CV_MAT_ELEM(*points, float, 0, j);
          CV_MAT_ELEM(*fitPoints, float, 1, k) = CV_MAT_ELEM(*points, float, 1, j);
          k++;
        }
      ...
      {
        // get distances
        mind = minp.x * curLineAbc[0] + minp.y * curLineAbc[1] + curLineAbc[2];
        maxd = maxp.x * curLineAbc[0] + maxp.y * curLineAbc[1] + curLineAbc[2];
    ...
    {  //#ifdef DEBUG_GET_STOP_LINES
      ...
      {
        mcvIntersectLineRThetaWithBB(curLineRTheta[0], curLineRTheta[1], cvSize(image->cols, image->rows), &line);
      ...
      {
        mcvIntersectLineRThetaWithBB(bestLineRTheta[0], bestLineRTheta[1], cvSize(image->cols, image->rows), &line);
  ...
  if (lineXY)
  {
    if (getEndPoints)
      *lineXY = bestEndPointLine;
      ...
      mcvIntersectLineRThetaWithBB(lineRTheta[0], lineRTheta[1], cvSize(image->cols - 1, image->rows - 1), lineXY);
  }
  if (lineScore)
    *lineScore = bestScore;
...
#define MCV_GET_NZ_POINTS(inMatType, outMatType)                                                                       \
  ...
  for (int i = 0; i < inMat->rows; i++)                                                                                \
    for (int j = 0; j < inMat->cols; j++)                                                                              \
      ...
      {                                                                                                                \
        ...
        CV_MAT_ELEM(*outMat, outMatType, 2, k) = (outMatType)CV_MAT_ELEM(*inMat, inMatType, i, j);                     \
        k++;                                                                                                           \
      }
  ...
  // check type
  if (CV_MAT_TYPE(inMat->type) == FLOAT_MAT_TYPE && CV_MAT_TYPE(outMat->type) == FLOAT_MAT_TYPE)
  {
    MCV_GET_NZ_POINTS(FLOAT_MAT_ELEM_TYPE, FLOAT_MAT_ELEM_TYPE)
  }
  else if (CV_MAT_TYPE(inMat->type) == FLOAT_MAT_TYPE && CV_MAT_TYPE(outMat->type) == INT_MAT_TYPE)
  {
    MCV_GET_NZ_POINTS(FLOAT_MAT_ELEM_TYPE, INT_MAT_ELEM_TYPE)
  }
  else if (CV_MAT_TYPE(inMat->type) == INT_MAT_TYPE && CV_MAT_TYPE(outMat->type) == FLOAT_MAT_TYPE)
  {
    MCV_GET_NZ_POINTS(INT_MAT_ELEM_TYPE, FLOAT_MAT_ELEM_TYPE)
  }
  else if (CV_MAT_TYPE(inMat->type) == INT_MAT_TYPE && CV_MAT_TYPE(outMat->type) == INT_MAT_TYPE)
  {
...
#define MCV_CUM_SUM(type)                                                                                              \
  ...
  if (inMat->rows == 1)                                                                                                \
    for (int i = 1; i < outMat->cols; i++)                                                                             \
      CV_MAT_ELEM(*outMat, type, 0, i) += CV_MAT_ELEM(*outMat, type, 0, i - 1);                                        \
  /*column vector*/                                                                                                    \
  else                                                                                                                 \
    for (int i = 1; i < outMat->rows; i++)                                                                             \
      CV_MAT_ELEM(*outMat, type, i, 0) += CV_MAT_ELEM(*outMat, type, i - 1, 0);
...
void mcvDrawLine(CvMat *image, Line line, CvScalar color, int width)
{
  cvLine(image, cvPoint((int)line.startPoint.x, (int)line.startPoint.y),
         cvPoint((int)line.endPoint.x, (int)line.endPoint.y), color, width);
}
...
{
  // draw the rectangle
  cvRectangle(image, cvPoint(rect.x, rect.y), cvPoint(rect.x + rect.width - 1, rect.y + rect.height - 1), color, width);
}
