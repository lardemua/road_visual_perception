  ...
  while (cont)
  {
    ...
    for (i = boxes.begin(); i != boxes.end(); i++)
    {
      for (j = i + 1; j != boxes.end(); j++)
      {
        ...
        case LINE_VERTICAL:
          //get one with smallest x, and compute the x2 - x1 / width of smallest
          //i.e. (x12 - x21) / (x22 - x21)
          overlap = i->box.x < j->box.x ? (i->box.x + i->box.width - j->box.x) / (float)j->box.width : (j->box.x + j->box.width - i->box.x) / (float)i->box.width;
        ...
        case LINE_HORIZONTAL:
          //get one with smallest y, and compute the y2 - y1 / height of smallest
          //i.e. (y12 - y21) / (y22 - y21)
          overlap = i->box.y < j->box.y ? (i->box.y + i->box.height - j->box.y) / (float)j->box.height : (j->box.y + j->box.height - i->box.y) / (float)i->box.height;
    ...
    if (maxOverlap >= groupThreshold)
    {
      ...
      *maxI = Box{cvRect(min((*maxI).box.x, (*maxJ).box.x),
                         ...
                         max((*maxI).box.height, (*maxJ).box.height)),
                  lin};
      //delete the second one
      boxes.erase(maxJ);
    }
