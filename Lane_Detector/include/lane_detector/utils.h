  ...
  for (auto i = boxes.begin(); i != boxes.end(); i++)
  {
    for (auto j = i + 1; j != boxes.end(); j++)
    {
      ...
      case LaneDetector::LINE_VERTICAL:
        ...
        overlap = i->box.x < j->box.x ? (i->box.x + i->box.width - j->box.x) : (j->box.x + j->box.width - i->box.x);
      ...
      case LaneDetector::LINE_HORIZONTAL:
        ...
        overlap = i->box.y < j->box.y ? (i->box.y + i->box.height - j->box.y) : (j->box.y + j->box.height - i->box.y);
...
inline void setMat(cv::Mat &inMat, cv::Rect mask)
{
  ...
  if (mask.y < 0)
    mask.y = 0;
  else if (mask.y >= inMat.rows)
    mask.y = inMat.rows - 1;
  if (mask.x < 0)
    mask.x = 0;
  else if (mask.x >= inMat.cols)
    mask.x = inMat.cols - 1;
